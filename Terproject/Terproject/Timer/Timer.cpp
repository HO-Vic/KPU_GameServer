...
void Timer::TimerThreadFunc()
{
	while (m_isRunning) {
		...
		case EV_RANDOM_MOVE:
		{
			ExpOver* ov = new ExpOver(OP_NPC_MOVE);
		...
		case EV_CHASE_MOVE:
		{
			...
			ExpOverBuffer* ov = new ExpOverBuffer(OP_NPC_CHASE_MOVE, reinterpret_cast<char*>(&targetId), sizeof(int));
			PostQueuedCompletionStatus(g_iocpNetwork.GetIocpHandle(), 1, ev.GetOwner(), reinterpret_cast<WSAOVERLAPPED*>(ov));
		}
		break;
		case EV_RESPAWN_NPC:
		{
			ExpOver* ov = new ExpOver(OP_NPC_RESPAWN);
			PostQueuedCompletionStatus(g_iocpNetwork.GetIocpHandle(), 1, ev.GetOwner(), reinterpret_cast<WSAOVERLAPPED*>(ov));
		}
		break;
		/*
