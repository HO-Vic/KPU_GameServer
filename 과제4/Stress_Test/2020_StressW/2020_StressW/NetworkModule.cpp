...
high_resolution_clock::time_point last_connect_time;	// 사용안함
...
struct CLIENT {
	...
	// Client에서 날아온 가장 최근의 Move Time
};

array<int, MAX_CLIENTS> client_map;		// 서버가 보낸 Client ID 와 Dummy Client ID 를 연결해주는 중계 컨테이너.
array<CLIENT, MAX_CLIENTS> g_clients;	// Dummy Client 관리 -> Server 내에서의 Client ID와 g_clients 의 ID와 다르다.
atomic_int num_connections;				// 
atomic_int client_to_close;				// 동접 접속 끊기 (오래 접속하고 정상 작동하고 있는 Client 부터 끊는다.)
atomic_int active_clients;

int			global_delay;				// ms단위, 1000이 넘으면 클라이언트 증가 종료
// 랙을 global_delay 에 기록한다, 한번에 증/감소하는 것이 아니라 천천히(1m/s씩) 반영된다.
...
void Adjust_Number_Of_Client()
{
	...
	if (active_clients >= MAX_TEST) return;		// 지금 최대동접이다.
	if (num_connections >= MAX_CLIENTS) return;	// 최대 클라에 도달 (현 알고리즘의 한계)
	...
	if (DELAY_LIMIT2 < t_delay) {
		...
		if (DELAY_LIMIT < t_delay) {	// 적절한 Delay 로 조절한다.
			delay_multiplier = 10;
			return;
		}
	if (max_limit - (max_limit / 20) < active_clients) return;

	increasing = true;	// 동접이 증가한다.
	last_connect_time = high_resolution_clock::now();
	...
	num_connections++;	// 현재 동접의 최대 인덱스를 증가시킨다.
fail_to_connect:
	return;
}
...
void Test_Thread()
{
	while (true) {
		...
		Adjust_Number_Of_Client();	// 첫 시작, 접속 (시간차를 두고 동접을 적당한 속도로 늘린다.)
		// 동접 증/감 속도도 Adjust_Number_Of_Client() 함수에서 결정, 처리
		...
		for (int i = 0; i < num_connections; ++i) {
			...
			// 지금 시간을 m/s 로 변환하고 time_since_epoch() (= 절대시간) 을 보내준다.
