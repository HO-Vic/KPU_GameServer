...
public:
	...
	atomic_bool isActive; //주위에 플레이어가 있는가?
...
{
	if (clients[from]._state != ST_INGAME)
		return false;
	...
	for (auto pl : old_vl) {
		if (0 == new_vl.count(pl)) {
			...
			if (0 != clients[pl]._view_list.count(npc._id)) {
				clients[pl]._view_list.erase(npc._id);
		...
		for (int i = MAX_USER; i < MAX_USER + MAX_NPC; ++i) {
			for (int j = 0; j < MAX_USER; j++)
				if (can_see(j, i)) {
					...
					PostQueuedCompletionStatus(h_iocp, 1, i, &ov->_over);
				}
		}
		auto end_t = chrono::system_clock::now();
		cout << chrono::duration_cast<chrono::microseconds>(end_t - start_t).count() << "ms" << endl;
...
void timer_thread() {
	while (true) {
		...
		if (timer_queue.try_pop(ev)) {
			if (ev.wakeup_time > currentTime) {
				timer_queue.push(ev); //다시 넣지 않고 처리해야됨
				continue;
			}
			switch (ev.event_id)
			{
