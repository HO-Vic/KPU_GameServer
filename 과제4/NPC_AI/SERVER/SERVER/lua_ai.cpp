...
chrono::system_clock::time_point moveTime = chrono::system_clock::now();
...
enum EVENT_TYPE { EV_RANDOM_MOVE, EV_RANDOM_MOVE_FACE, EV_RANDOM_MOVE_FINISH };
...
public:
	...
	atomic_bool	_is_player_touch = false;
	int _id;
	SOCKET _socket;
	short	x, y;
...
int API_helloSendMessage(lua_State* L)
{
	...
	bool old_state = false;
	if (false == atomic_compare_exchange_strong(&clients[my_id]._is_player_touch, &old_state, true))
		return 0;

	TIMER_EVENT ev{ my_id, chrono::system_clock::now()+500ms, EV_RANDOM_MOVE_FACE, user_id };
	TIMER_EVENT ev1{ my_id, chrono::system_clock::now() + 1s + 500ms, EV_RANDOM_MOVE_FACE, user_id };
	TIMER_EVENT ev2{ my_id, chrono::system_clock::now() + 2s + 500ms, EV_RANDOM_MOVE_FINISH, user_id };
	timer_queue.push(ev);
...
int API_ByeSendMessage(lua_State* L)
{
	...
	bool old_state = true;
	atomic_compare_exchange_strong(&clients[my_id]._is_player_touch, &old_state, false);
	lua_settop(L, 0);

	TIMER_EVENT ev{ my_id, chrono::system_clock::now() + 1s, EV_RANDOM_MOVE_FACE, user_id };
	timer_queue.push(ev);
...
void do_timer()
{
	while (true) {
		...
		if (true == timer_queue.try_pop(ev)) {
			...
			case EV_RANDOM_MOVE:
			{
				if (!clients[ev.obj_id]._is_player_touch) {
			...
			case EV_RANDOM_MOVE_FINISH:
			{
				...
				PostQueuedCompletionStatus(h_iocp, 1, ev.obj_id, &ov->_over);
				//bool old_state = true;
				//atomic_compare_exchange_strong(&clients[ev.obj_id]._is_player_touch, &old_state, false);
				OVER_EXP* byeOv = new OVER_EXP;
				byeOv->_comp_type = OP_AI_BYE;
				byeOv->_ai_target_obj = ev.target_id;
				PostQueuedCompletionStatus(h_iocp, 1, ev.obj_id, &byeOv->_over);
			}
			break;
			}
