...
//동접 약 500정도 - 밀도가 너무 커서 문제
//섹터를 이용해서 분할해서 하자 - 좀 나아짐
//can see가 아니라 랜덤 무브 영역안에 들어간다면? 으로 바꿔야 됨
...
public:
	...
	mutex	_vl;
	int		last_move_time;
	lua_State* _L; // 스택에 순서 꼬이면 안되니 -> mutex
	mutex _ll;
public:
	SESSION()
	{
...
void WakeUpNPC(int npc_id, int waker)
{
	...
	ex_over->_comp_type = OP_AI_HELLO;
	ex_over->ai_target_obj = waker;
	PostQueuedCompletionStatus(h_iocp, 1, npc_id, &ex_over->_over);
	if (clients[npc_id]._is_active) return;
	bool old_state = false;
	if (false == atomic_compare_exchange_strong(&clients[npc_id]._is_active, &old_state, true)) return;//? 뭐노 - 깨운 쓰레드가 밑에 행동할 수 있게, 다른 쓰레드랑 동시에 해버리면 밑에 이벤트 중첩돼서 행동을 바꾼 쓰레드만 밑에 실행
	...
	while (true) {
		...
		case OP_NPC_MOVE:
		{
			...
			for (int j = 0; j < MAX_USER; ++j) {
				...
				if (can_see(static_cast<int>(key), j)) { // 주위에 플레이어가 있는지?
					keep_alive = true;
					break;
				}
			}
			if (true == keep_alive) { // 주변에 플레이어가 있네
		...
		case OP_AI_HELLO:
		{
			...
			lua_getglobal(clients[key]._L, "event Player Move");
			lua_pushnumber(clients[key]._L, ex_over->ai_target_obj);
			lua_pcall(clients[key]._L, 0, 0, 0);
			lua_pop(clients[key]._L, 1);
...
int API_SendMessage(lua_State* L)
{
	...
	lua_pop(L, 4);
	clients[user_id].send_chat_packet(user_id, mess);
	return 0;
}
...
void do_timer()
{
	while (true) {
		...
		if (timer_queue.try_pop(ev)) {
			if (ev.wakeup_time > current_time) {// 아직 실행시간이 아니네?
				timer_queue.push(ev);		// 최적화 필요
				// timer_queue에 다시 넣지 않고 처리해야 한다.
				this_thread::sleep_for(1ms);//잠깐 기다렸다가?? 왜 기다림
				continue;
			}
			switch (ev.event_id) {			//시간이 됐다면
			case EV_RANDOM_MOVE:			//이벤트 실행
				OVER_EXP* ov = new OVER_EXP;
				ov->_comp_type = OP_NPC_MOVE;
				PostQueuedCompletionStatus(h_iocp, 1, ev.obj_id, &ov->_over);
				break;
			}								//작업이 실행되면 바로 이벤트 뽑기
			continue;						//성공했다면 바로 다시 실행될 수 있게
		}
		this_thread::sleep_for(1ms);		//뽑을 놈이 없다면 좀 기다렸다가 뽑이라
	}
}
